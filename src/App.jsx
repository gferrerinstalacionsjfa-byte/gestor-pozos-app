...
async function parseMuntatgesFromWorkbook(assocWb) {
  ...
  for (let r = 1; r < sheet.rows.length; r++) {
    ...
    if (cable === null) {
      ...
      notMounted.push({ pozo: String(pozo), eui, cota, installer, vigilant, date, acabat, remesa });
      continue;
    }
    rows.push({ pozo: String(pozo), eui, cota, cable, installer, vigilant, date, acabat, remesa });
  }
...
function MuntatgesView() {
  ...
  // una columna es visible si l'usuari no l'ha amagat manualment, i no és redundant amb l'agrupació actual
  const colOn = (key) => {
    if (!visibleCols[key]) return false;
    if (key === "illa" && groupBy === "illa") return false;
    if (key === "installer" && groupBy === "installer") return false;
    if (key === "vigilant" && groupBy === "vigilant") return false;
  ...
  const buildExportTable = () => {
    ...
    groups.forEach((g) => {
      sortWells(g.wells).forEach((r) => {
        ...
        rows.push(columns.map((c) => values[c.key]));
      });
    });
    return { head, rows };
  };
        ...
        {rows && (
          <>
            <div style={styles.statsRow}>
              ...
              {groupBy !== "none" && (
                <Stat
                  label={groupBy === "illa" ? "Illes" : groupBy === "vigilant" ? "Vigilants" : "Instal·ladors"}
                  value={groups.length}
                />
              )}
            </div>
